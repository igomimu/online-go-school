...
test.describe('検討モードの碁盤操作(pokekata踏襲)', () => {
  ...
  test('着手→Delete取り消し→ホイールで手順送り/戻りが機能する', async () => {
    ...
    await expect(review.getByText('1手目')).toBeVisible({ timeout: 5_000 });

    // Deleteキーで取り消し → 0手目に戻る
    await review.keyboard.press('Delete');
    await expect(review.getByText('0手目')).toBeVisible({ timeout: 5_000 });

    // 再度着手して Ctrl+Z でも取り消せることを確認
    await board.locator('[data-cell="4-4"]').click();
    await expect(review.getByText('1手目')).toBeVisible({ timeout: 5_000 });
    await review.keyboard.press('Control+z');
    await expect(review.getByText('0手目')).toBeVisible({ timeout: 5_000 });
    ...
    await board.hover();
    await review.mouse.wheel(0, -100); // 上スクロール = 戻る
    await expect(review.getByText('0手目')).toBeVisible({ timeout: 5_000 });

    await board.hover();
    await review.mouse.wheel(0, 100); // 下スクロール = 進む
    await expect(review.getByText('1手目')).toBeVisible({ timeout: 5_000 });
  });
  ...
  test('矢印キーで手順送り/戻り、チャット入力中は無効化される', async () => {
    ...
    const showPanelButton = review.getByRole('button', { name: '操作パネルを表示' });
    if (await showPanelButton.count() > 0) {
      await showPanelButton.click();
    }
    const chatInput = review.locator('input[type="text"]').first();
...
test.describe('検討モードの狭い画面', () => {
  ...
  test('390×667 で操作列が画面内に収まり、見出しが語の途中で折れない', async () => {
    ...
    // 検討は別ウィンドウに出るので、そのウィンドウを狭い画面にして測る
    const review = await loadSgfForReview(page, '(;FF[4]GM[1]SZ[9])');
    await review.setViewportSize({ width: 390, height: 667 });
    await review.getByTestId('go-board').waitFor({ timeout: 15_000 });
    await review.waitForTimeout(600);

    const m = await review.evaluate(() => {
