  ...
  return (
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50" onClick={onClose}>
      <div className="glass-panel p-6 w-full max-w-sm" onClick={(e) => e.stopPropagation()}>
        ...
        {sourceId !== undefined && (
          <p className="text-xs text-muted mb-3">問題 #{sourceId}</p>
        )}

        {done ? (
          <div className="text-center py-4">
            <p className="text-ink">報告を受け付けました。</p>
            <p className="text-xs text-muted mt-1">確認して修正します。</p>
          ...
          <div>
            <label className="block text-xs text-muted mb-1">気になった点（任意）</label>
            <textarea
              value={reason}
              ...
              rows={3}
              maxLength={500}
              className="w-full resize-none bg-ink/5 border border-field-line rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-accent"
            />
            {error && <p className="text-xs text-alert-text mt-2">{error}</p>}
