...
- 検証: 関連Vitest 55件、TypeScript/Vite本番ビルド、ESLint成功。Chromium 1522×934・800×600で、映像の欠け、横送り、設定パネルの上下端、開閉前後のヘッダー高を確認。対局盤は自分の手番・相手の手番の両状態で操作欄が同じ固定高を維持するテストを追加。
- 本番反映: 対局中にPWA更新が入ると自動再読み込みが発生するため、テスト終了まではpushを保留する。
