...
- 検証: 全体Vitest 57ファイル549件（Supabase環境変数が必要な既存useGameManagerファイルのみ除外）、TypeScript/Vite本番ビルド、ESLint成功。Chromium 1522×934・800×600で、映像の欠け、横送り、設定パネルの上下端、開閉前後のヘッダー高を確認。対局盤は自分の手番・相手の手番の両状態で操作欄が同じ固定高を維持し、中断・再開は講師だけ操作できるテストを追加。
