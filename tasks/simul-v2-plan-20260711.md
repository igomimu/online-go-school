...
## 追補（2026-07-14）: 入口の一本化（三村さん実機テストのフィードバック）

実機テストで判明した問題: 通常の「対局作成」から先生対局を始めると**全画面碁盤が自動で開き（d976887）、
対局追加も他の画面も操作できない**。多面打ちは「多面打ちボタン→専用ダイアログ」経由でしか成立しなかった。

三村さん指示による設計変更（本日実装済み）:
- **多面打ちボタンは廃止**。多面打ち専用の対局追加ダイアログ（SimulAddGameDialog）も削除。
- **通常の「対局作成」を重ねるだけで盤が増える**。先生自身が対局者の対局は、面数に関わらず
  常に多面打ちビュー（1盤表示＋ローテーション）で開く。全画面碁盤には入れない。
- 多面打ちは同時に始まるとは限らない。後から「対局作成」で追加していく運用が前提。
- 再入経路: ダッシュボードのサムネイル / 生徒一覧の「開く」で、自分の対局なら多面打ちビューへ。
- SimulGrid の「対局を追加」ボタンは通常の対局作成ダイアログ（時計設定等フル機能）を開く。

変更ファイル: App.tsx（handleCreateGame/handleResumeGame）、TeacherDashboard.tsx（ボタン削除・再入配線）、
SimulGrid.tsx（onOpenGameCreation化）、SimulAddGameDialog削除、E2E 6spec追従（helpers waitForSimulBoard/closeSimulToHome追加）。
